...
test.describe("tenant governance", () => {
  ...
  test("renders projects, users, quotas, and audit pages", async ({ page }) => {
    expect(previewUrl).toBeDefined();

    await page.goto(`${previewUrl ?? ""}/organization/projects`);
    await expect(page.getByRole("heading", { name: "Projects" })).toBeVisible();
    await expect(page.getByRole("table")).toBeVisible();

    await page.goto(`${previewUrl ?? ""}/organization/users`);
    await expect(page.getByRole("heading", { name: "Users" })).toBeVisible();
    await expect(page.getByRole("table")).toBeVisible();

    await page.goto(`${previewUrl ?? ""}/organization/quotas`);
    await expect(page.getByRole("heading", { name: "Quotas" })).toBeVisible();

    await page.goto(`${previewUrl ?? ""}/organization/audit`);
  ...
  test("creates an API credential and reveals the one-time secret", async ({ page }) => {
    ...
    await page.getByLabel("Name").fill("e2e-test-credential");
    await page.getByLabel("Project ID").fill("project-1");
    await page.getByRole("button", { name: "Create", exact: true }).click();

    await expect(page.getByText("API credential created")).toBeVisible();
    await expect(page.getByLabel("API credential secret")).toHaveValue(/secret-/);
  });

  test("rejects cross-project access with a safe error", async ({ page }) => {
    expect(previewUrl).toBeDefined();

    await page.goto(`${previewUrl ?? ""}/organization/projects/project-99`);
    await expect(
      // Cloudscape Alert header is not a semantic heading, so match visible text.
      page.getByText(/Could not load project|Not found|Access denied|Error/),
    ).toBeVisible();
  });
});
